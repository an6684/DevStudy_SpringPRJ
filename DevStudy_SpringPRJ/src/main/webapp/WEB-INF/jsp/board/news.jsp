...
<body>
    ...
    <section id="board-section">
        <div class="board-list">
                        ...
                        <tr>
                            ...
                            <td>
                            	...
                            	<a href="noticeDetail?id=${nl.content_id}">
                            		${nl.title}
	                            	<c:choose>
										<c:when test="${not empty nl.filepath}">
                            ...
                            <td>${nl.writedid}</td>
                            <td><fmt:formatDate pattern="yyyy년 MM월 dd일 hh시 mm분" value="${nl.regdate}" /></td>
                            <c:choose>
								<c:when test="${userRank eq '1'}">
									<td><input id='del-btn' type="button" value="삭제"  onclick="confirmDelete(${nl.content_id})"/></td>
								</c:when>
								<c:otherwise>
								</c:otherwise>
							</c:choose>
            ...
            <c:if test="${not empty sessionScope.userid && (sessionScope.userrank eq '1' or sessionScope.userrank eq '2')}">
